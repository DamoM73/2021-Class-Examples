...
def display_query(results):
    # displays the results from an sql cursor
    for row in results:
        print(row[0])
...
def query_db(query):
    ...
    with sqlite3.connect(DB_FILE) as database:
        ...
        #display_query(cursor.execute(query))
        export_csv(cursor.execute(query))

def export_csv(results):
    # saves the proved data to csv file
    
    with open(PATH + 'client_export.csv', 'w', newline='') as file:
...
query = """
        ...
        WHERE activity IN (
            SELECT activity
            FROM Classes
            WHERE instructor = "Cox Kimberley"
        )
        """
query_db(query)
